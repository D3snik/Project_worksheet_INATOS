...
def render_topo(ferramenta_ativa: str):
    folha_classes = "header-link is-active" if ferramenta_ativa == "folha" else "header-link"
    notas_classes = "header-link is-active" if ferramenta_ativa == "notas" else "header-link"

    st.markdown(
        f"""
        <header class="app-header">
            <a class="brand-link" href="?tool=home" target="_self">
                <span class="brand-word">Central</span><span class="brand-accent">PDF</span>
            </a>
            <nav class="header-nav" aria-label="Navegação principal">
                <a class="{folha_classes}" href="?tool=folha" target="_self">EXTRAÇÃO DE FOLHA</a>
                <a class="{notas_classes}" href="?tool=notas" target="_self">NOTAS</a>
            </nav>
        </header>
        """,
...
def render_cards():
    ...
    with col1:
        st.markdown(
            f"""
            <a class="tool-card-link" href="?tool=folha" target="_self">
                <div class="tool-card">
    ...
    with col2:
        st.markdown(
            """
            <a class="tool-card-link" href="?tool=notas" target="_self">
                <div class="tool-card">
                    <div class="tool-icon-wrap">
                        <div class="tool-icon-default">
